...
import type { EstadoAvisoAdmin } from "@/types/estado-aviso"

export default function EstadosPage() {
  const router = useRouter()
  const [session, setSession] = useState<AuthResponse | null>(null)
  const [estados, setEstados] = useState<EstadoAvisoAdmin[]>([])
  const [editando, setEditando] = useState<EstadoAvisoAdmin | null>(null)
  const [nombre, setNombre] = useState("")
  const [error, setError] = useState("")

  const cargar = useCallback(async (current: AuthResponse) => {
    const response = await fetch("/api/pala/admin/estados/avisos", { headers: authHeader(current), cache: "no-store" })
  ...
  async function guardarNombre() {
    ...
    const response = await fetch(`/api/pala/admin/estados/avisos/${editando.id}`, {
      method: "PATCH",
  ...
  return (
    <AdminShell mail={session.mailUsuario}>
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-lg"><FileText className="h-5 w-5 text-sky-600" /> Estados de aviso</CardTitle>
          ...
          <div className="overflow-x-auto rounded-xl border">
            <table className="w-full min-w-[720px] text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase text-slate-500">
                <tr><th className="px-4 py-3">Nombre</th><th className="px-4 py-3">Código interno</th><th className="px-4 py-3">Avisos actuales</th><th className="px-4 py-3">Acciones</th></tr>
              </thead>
              <tbody className="divide-y">
                {estados.map((estado) => (
                  <tr key={estado.id}>
                    ...
                    <td className="px-4 py-4 tabular-nums text-slate-600">{estado.avisosActuales}</td>
