...
export async function GET(
  ...
  const { reclutadorId } = await params
  ...
  try {
    const response = await fetch(
      `${PALA_API_URL}/api/reclutadores/${reclutadorId}/avisos/soporte/empresas-activas`,
      {
        cache: "no-store",
        headers: {
    ...
    console.error(
      "[/api/pala/reclutadores/[reclutadorId]/avisos/soporte/empresas-activas] Error al contactar el backend:",
      err
    )
    return NextResponse.json(
